...
and soil conditions rather than the panels themselves.
...
Map](https://agrivoltaicsmap.ise.fraunhofer.de/) and from satellite
basemap imagery in QGIS:

- `sites` — 25 agrovoltaic facility polygons across Bavaria and
  Baden-Württemberg
- `fields` — 25 paired reference agricultural field polygons
- `panels` — 655 photovoltaic panel array footprints, linked to sites
  via `site_id`
...
needs to be run once — skips files already present. Requires an internet
...
reference field. The shaded band shows mean ± 1 standard deviation.
...
location overview and a panel footprint overview. Note: this map shows
the full unmasked scene for visual clarity — the underlying statistics
from `load_site_results()` are based on masked data that excludes panel
pixels.
...
Compares overhead vs interspace agrovoltaic systems. Load and combine
results from multiple sites first.

``` r
list_system_types()
```

Then load and combine results from multiple sites:

``` r
results_multi <- dplyr::bind_rows(
  load_site_results(site_id = 1),
  load_site_results(site_id = 2),
  load_site_results(site_id = 12),
  load_site_results(site_id = 16)
...
If no suitable scene is found, try raising the cloud cover threshold
(use with caution, as cloudier scenes reduce index quality):
...
**Important:** `site_sf`, `field_sf`, and `site_id` must always refer to
...
- Pixels covered by photovoltaic panels (from the `panels` dataset) are
  masked out before computing the statistics returned by
  `load_site_results()`, `plot_indices()`, `compare_system_types()`, and
  `plot_system_types()`. The maps produced by `map_indices()`
  intentionally show the full unmasked scene for visual clarity, with
  panel footprints displayed separately in the “Panel footprint”
  overview panel.
- Cached results are stored in a local `cache/` folder, which is not
  included in the package repository. Run `download_cache()` to populate
  it from Zenodo.
- `map_indices()` reads from a separate `cache_unmasked/` folder by
  default, since its purpose is visual display rather than statistics.
